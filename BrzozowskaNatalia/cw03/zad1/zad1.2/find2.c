...
int func(const char *name,const struct stat * el_stat, int type, struct FTW *tfw){
    ...
    if(S_ISDIR(el_stat->st_mode) && !S_ISLNK(el_stat->st_mode)){

                int child_pid = vfork();

                if(child_pid==0){                                                // child process is doing this
                    printf("PROCESSES path: %s ,PID: %d \n",rel_dir, (int)getpid());
                    execlp("ls","ls","-l",name,NULL);                             //arguments as list (l), (p) - file with program to run is in PATH
                }
                wait(NULL);
